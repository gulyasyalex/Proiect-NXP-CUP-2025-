...
class RotaryEncoder
{
  ...
  public:
	...
	void loop() {
		// Read the current state of DT and CLK pins
		const unsigned char pinstate = (digitalRead(DT_Pin) << 1) | digitalRead(CLK_Pin);
		
		// Update the internal state using the state transition table
		state = ttable[state & 0x07][pinstate];
		
		// If a direction is detected (DIR_CW or DIR_CCW), trigger the callback
		if (state & (DIR_CW | DIR_CCW)) {
			RotaryCallback();
		}
	}
